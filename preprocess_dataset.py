...
def process_data(test_images_file, annotations_file, class_descriptions_file, nrows=None):
    test_images_df = pd.read_csv(test_images_file, nrows=nrows)
    annotations_df = pd.read_csv(annotations_file)

    class_descriptions = pd.read_csv(class_descriptions_file)
    label_display_map = dict(zip(class_descriptions['LabelName'], class_descriptions['DisplayName']))

    confident_annotations = annotations_df[annotations_df['Confidence'] == 1]

    merged_df = pd.merge(confident_annotations, test_images_df[['ImageID', 'OriginalURL']], on='ImageID')
...
def main():
    ...
    joined_df.head(10000).to_csv('urls.csv', index=False, columns=['url', 'image_labels', 'bbox_labels'])
